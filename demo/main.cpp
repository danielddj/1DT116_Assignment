...
{
    printf("Usage: %s [--timing-mode|--export-trace[=export_trace.bin]] [--max-steps=100] [--help] [--cuda|--simd|--omp=[number_of_threads]|--threads=[number_of_threads]|--pthread|--seq] [scenario filename]\n", command);
...
int main(int argc, char *argv[])
{
    ...
    Ped::Model::numberOfThreads = (std::thread::hardware_concurrency() > 0) ? std::thread::hardware_concurrency() : 1;
    ...
    while (1)
    {
        ...
        case 'o':
            ...
            if (optarg != NULL)
            {
                ...
                std::cout << "Option --omp set to: " << optarg << std::endl;
            }
            else
            {
        ...
        case 'T':
            ...
            if (optarg != NULL)
            {
                // If an argument is provided set it as the number of threads
                Ped::Model::numberOfThreads = std::stoi(optarg);
                std::cout << "Option --threads set to: " << optarg << std::endl;
            }
            else
            {
                std::cout << "Option --threads activated\n";
            }
            implementation_to_test = Ped::THREADS;
            break;
        case 'p':
            // Handle --pthread
