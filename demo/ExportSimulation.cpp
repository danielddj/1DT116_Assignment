...
ExportSimulation::ExportSimulation(Ped::Model &model_, int maxSteps,
  ...
  file.write(reinterpret_cast<const char *>(&maxSimulationSteps),
             sizeof(maxSimulationSteps));
}
...
void ExportSimulation::serialize() {
  ...
  // file.write(reinterpret_cast<const char*>(&heatmap_elements),
  // sizeof(heatmap_elements));
  ...
  file.write(reinterpret_cast<const char *>(&heatmap_start),
